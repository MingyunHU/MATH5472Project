function [Z_i_new, R_i_A, R_i_B, F_i_A, F_i_B] = ALS_Anderson_soft(W, M, Z_i, R_i_1_A,R_i_1_B, F_i_1_A,F_i_1_B, maxlen, len_A, lambda)
A = Z_i(1:len_A,:);
B = Z_i(len_A+1:end,:);
Y = W.*M+(ones(size(W))-W).*(A*B');
B_update = Y'*A/(A'*A+lambda*eye(size(A'*A)));
b_i = B(:);
f_b_i = B_update(:);
r_b_i = f_b_i-b_i;
R_i_B = [R_i_1_B, r_b_i];
...
A_update = Y*B_update/(B_update'*B_update+lambda*eye(size(B_update'*B_update)));
a_i = A(:);
f_a_i = A_update(:);
r_a_i = f_a_i-a_i;
R_i_A = [R_i_1_A, r_a_i];
